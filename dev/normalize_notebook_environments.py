...
def repair_hamiltonian_phase_maps(nb):
    # Preserve the revised scientific-status contract in notebook 06.
    if any("hamiltonian_yamada_phase_maps_v5_audited_spatial_only" in text(c)
           for c in nb["cells"]):
        set_text(nb["cells"][0], '# Hamiltonian volume and spatial-invariant audit\n\n> Advanced numerical study; the complete scan is not part of ordinary notebook CI.\n\nThe corrected route retains raw per-cell volume homology, extraction status and,\nwhen available, an audited spatial or fixed-diagram Yamada evaluation. Empty,\ncavity-containing or boundary-truncated masks remain explicitly unsupported in\nthe default closed graph-spine regime. Extraction and projection failures do not\nbecome point phases or abstract-polynomial substitutes.\n\nThe historical 7,140-cell phase figure used mixed quantities and display\npostprocessing. It is retained only as legacy audit material. The revised\nmanuscript uses a separate 1,309-cell, 64-cubed homology/status pilot; it is not a\nYamada classification or a resolution-converged continuum phase diagram.\n\nThe configured full notebook scan has 60 lambda and 50 candidate energy values\nat 120 cubed. It can be expensive. Per-row JSON retains failures and unsupported\ncells. Historical classification reports, filtering and abstract contraction\ngrouping are disabled by default. Install the nodal, viz and notebook extras\nand inspect the source/provenance before a new run.')
        audited_guides = {'phase-map-configuration-guide': "## Configuration, cost and provenance\n\nThe setup reports the imported package and native backend and specifies the\nfinite sampling grid, spatial resolution, retry policy and output locations.\nThe notebook's row/cache loop is sequential; N_JOBS is a retained configuration\nvariable, not evidence of an executed parallel scan. The separate revised\nhomology pilot explicitly uses two worker processes.\n\nCache keys identify the audited evaluation policy and boundary convention.\nChanging these controls requires new numerical records. A finite-grid result\ndoes not establish a continuum phase boundary.", 'phase-map-record-guide': '## What one phase-map record means\n\nEach successful cell stores the sampled parameters, graph size, connected\ncomponents, cycle rank, degree sequence, total edge-geometry samples, exact\nYamada expression, and a stable phase signature. A failed extraction or projection stores an explicit unavailable result. Empty,\ncavity-containing or boundary-truncated volumes are outside the default closed\ngraph-spine regime. No extraction failure becomes a point, and no abstract graph\npolynomial is substituted for a spatial invariant. Higher-valence evaluations\nare retained as diagram-dependent quantities with projection metadata.\n\nRaw records remain separate from display arrays. Display filtering and legacy\nabstract-contraction grouping are disabled by default. If unavailable cells are\npresent, phase-label rendering fails explicitly; use their retained status masks\nrather than coloring them as successful phases. The archived historical figures\nare not regenerated or validated by these code corrections.\n\n`make_yamada_phase_map(...)` is the reusable application API. The longer code\nbelow adds the publication-specific extraction retries, cache layout,\nstabilization, figures, and validation rules. Keep that distinction in mind when\ncopying code into a new project.\n\n\nThe corrected default run saves the full requested energy range if no validated\nall-lambda isolated-vertex row occurs. The older automatic phase-class reports\nand figures are opt-in through RUN_LEGACY_CLASSIFICATION_REPORTS; they must not\npresent the historical 7,140-cell output as a validated spatial classification.\nThe revised manuscript instead uses a separately archived 64-cubed homology/status\npilot, which is a finite-grid diagnostic rather than a Yamada classification.\n', 'phase-map-audit-guide': '## Inspect raw records before any classification\n\nInspect volume b0, b1 and b2, boundary contact, mask-to-skeleton agreement,\ngraph homology, evaluation kind, projection metadata and each error. No\nunavailable cell receives a successful phase label. If no validated terminal\nisolated-vertex row is found, the full requested energy range is retained.\n\nLegacy classification reports are opt-in and cannot validate the old output.\nUse the separately archived homology/status pilot for the revised figure.', 'phase-map-figure-guide': '## Raw data and optional historical display code\n\nThe raw per-cell ledger is the numerical result. Display filtering is off by\ndefault; any optional filtered array is stored separately with its reassignment\nmask. Abstract contraction groups are not spatial-embedding equivalence classes\nand are disabled by default. Legacy figure cells are guarded by\nRUN_LEGACY_CLASSIFICATION_REPORTS. An unavailable cell requires a status mask,\nnot an invariant color.', 'interactive-plotly-qa-markdown': '## Optional historical interactive geometry display\n\nThe legacy Plotly display is opt-in. Its archived filtered arrays and\nrepresentative geometries do not recover missing raw parameter cells and do not\ncertify the original finite-volume topology. The corrected path exports raw\nlabels separately from any requested display labels and reassignment mask.', 'finalnodal': '## Verification scope\n\nThe corrected default run retains raw records and explicit unsupported/error\nstates. Legacy classification verification is disabled by default. The new\nhomology/status pilot records finite-grid checks only; passing these checks is\nnot a regular-neighborhood, embedding-isotopy or continuum-convergence proof.'}
        for cell in nb["cells"]:
            if cell.get("id") in audited_guides:
                set_text(cell, audited_guides[cell["id"]])
        return
