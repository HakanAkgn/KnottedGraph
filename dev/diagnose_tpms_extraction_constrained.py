    ...
    for family in tpms_families(dimension=dimension,thresholds=(0.,.105,.3)):
        ...
        for lam in (0.,.25,.5,.75,1.):
            ...
            for c in family.thresholds:
                ...
                if row['mask']['b2'] != 0:
                    row['classification_status']='unsupported_void_components'
                    row['seconds']=perf_counter()-started
                    rows.append(row)
                    continue
                row['classification_status']='evaluated_with_homology_constraints'
                try:
                    ...
                    selected=persistent_extract(coords,adj,expected_components=row['mask']['b0'],expected_cycle_rank=row['mask']['b1'])
                ...
                row['seconds']=perf_counter()-started
                rows.append(row)
        (OUT/'tpms_extraction_constrained_diagnosis.json').write_text(json.dumps({'source_sha256':source,'records':rows},indent=2))
