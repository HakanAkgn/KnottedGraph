...
def energy_axis(start: float, stop: float, step: float = 0.01) -> tuple[float, ...]:
    ...
    while current <= float(stop) + 1e-12:
        ...
        current += float(step)
    if not values or not math.isclose(
        values[-1], float(stop), rel_tol=0.0, abs_tol=1e-10
    ):
...
def adaptive_probe_indices(
    family: MaterialFamily, adaptive_energy_step: float
) -> list[int]:
    ...
    for landmark in family.landmark_energies:
        if energies[0] - 1e-12 <= float(landmark) <= energies[-1] + 1e-12:
            nearest = min(
                range(len(energies)),
                key=lambda idx: abs(energies[idx] - float(landmark)),
            )
            indices.add(nearest)
            if nearest > 0:
...
def contiguous_energy_runs(
    energies: tuple[float, ...], native_step: float = 0.01
) -> list[tuple[float, ...]]:
    ...
    for run in contiguous_energy_runs(missing_energies):
        run_family = dataclasses.replace(
            ...
            landmark_energies=tuple(
                value
                for value in family.landmark_energies
...
def connected_components_for_label(
    labels: np.ndarray, phase_id: int
...
def nearest_large_phase(
    ...
    target_phase, _ = min(
        ...
        key=lambda item: (
            float(
                np.sum((np.asarray(item[1], dtype=float).mean(axis=0) - center) ** 2)
            ),
            -len(item[1]),
            int(item[0]),
        ),
                ...
                for i, j in component:
                    for di, dj in ((1, 0), (-1, 0), (0, 1), (0, -1)):
                        ...
                        if 0 <= ni < stable.shape[0] and 0 <= nj < stable.shape[1]:
                            ...
                            if (
                                neighbor != int(phase_id)
                                and neighbor in large_phase_ids
                            ):
                                neighbor_counts[neighbor] += 1
            if strategy == "below":
                replacement = closest_lower_large_phase(
                    component, int(phase_id), large_components
                )
                if replacement is None:
                    replacement = nearest_large_phase(
                        component, int(phase_id), large_components
                    )
...
def apply_signature_phase_merges(
    ...
    for source_signature, target_signature in PHASE_SIGNATURE_MERGES.get(
        str(family_key), {}
    ).items():
...
def remap_labels_contiguous(
    ...
    remapped_lookup = {
        new: label_lookup.get(old, f"phase {old}") for old, new in old_to_new.items()
    }
    return remapped, remapped_lookup


def phase_grid(
    records: list[PhaseCell], lambdas: np.ndarray, energies: tuple[float, ...]
...
def write_static_overview(
    ...
    for ax, family, panel in zip(axes.ravel(), families, ["(a)", "(b)", "(c)", "(d)"]):
        ...
        ax.pcolormesh(
            lambdas, family.energies, labels, shading="nearest", cmap=cmap, norm=norm
        )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("_build/new_phase_maps/materials"),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Use this many evenly spaced E samples; 0 retains the research grid.",
    )
    ...
    parser.add_argument(
        "--min-stable-cells", type=int, default=DEFAULT_MIN_STABLE_CELLS
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        if DEFAULT_ISLAND_MERGE_STRATEGY in {"below", "nearest"}
        else "below",
...
def extend_existing_dataset(args: argparse.Namespace) -> None:
    ...
    family_dimensions = {
        str(item["key"]): int(item["dimension"]) for item in summary["families"]
    }
    family_lookup = {
        family.key: family
    ...
    for family in families:
        ...
        counts_by_energy = Counter(
            round(float(record.energy), 12) for record in records_by_family[family.key]
        )
        partial_rows = {
            energy: count
            for energy, count in counts_by_energy.items()
    ...
    png_path = write_static_overview(
        args.output_dir, families, lambdas, stable_by_family
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.dimension < 8 or args.lambda_count < 2 or args.workers < 1:
        raise ValueError("Use dimension >= 8, lambda-count >= 2 and workers >= 1")
    if args.energy_count < 0 or args.energy_count == 1:
        raise ValueError("--energy-count must be 0 (full grid) or at least 2")
    if args.energy_count and (args.reuse_records or args.extend_existing_records):
        raise ValueError("--energy-count applies only to a new scan")
    if args.reuse_records and args.extend_existing_records:
        raise ValueError(
            "Choose either --reuse-records or --extend-existing-records, not both"
        )
    ...
    if args.reuse_records:
        ...
        families = tuple(
            dataclasses.replace(
                family,
                energies=tuple(
                    sorted({r.energy for r in records_by_family[family.key]})
                ),
            )
            for family in families
        )
            ...
            dataclasses.replace(
                ...
                energies=tuple(
                    ...
                    for e in np.linspace(
                        family.energies[0], family.energies[-1], args.energy_count
                    )
                ),
            )
            for family in families
    ...
    for family in families:
        print(
            f"\n[{family.key}] {family.title}: varying {family.lambda_param}",
            flush=True,
        )
