...
def positive_cube_span(width: float) -> tuple[tuple[float, float], tuple[float, float], tuple[float, float]]:
    value = float(width)
    ...
    return (
        FigureSpec(
            ...
            surfaces=(
                ...
                SurfaceSpec("tib2_mid", "2.8 eV < E < 3.9 eV", 3.2, cube_span(1.5), 200, opacity=0.60, old_notebook_cell=13),
                SurfaceSpec("tib2_high", "3.9 eV < E", 4.0, cube_span(1.5), 200, opacity=0.60, old_notebook_cell=16),
            ),
        ),
        FigureSpec(
            key="ti3al",
        ...
        FigureSpec(
            ...
            surfaces=(
                ...
                SurfaceSpec("yh3_high", "0.2 eV < E", 0.025, cube_span(1.0), 200, opacity=0.60, old_notebook_cell=56),
            ),
        ),
        FigureSpec(
            key="co2mnga",
            ...
            surfaces=(
                ...
                SurfaceSpec("co2mnga_high", "1.15 eV < E", 1.2, cube_span(2.05 * math.pi), 200, opacity=0.60, old_notebook_cell=67),
            ),
        ),
    )
...
def mesh_bounds(mesh: pv.PolyData, fallback_span: tuple[tuple[float, float], ...]) -> tuple[tuple[float, float], tuple[float, float], tuple[float, float]]:
    if mesh.n_points == 0:
...
def camera_from_old_view(elevation_deg: float, azimuth_deg: float, distance: float = 1.75) -> dict[str, dict[str, float]]:
...
def mesh_trace(mesh: pv.PolyData, surface: SurfaceSpec, fig_spec: FigureSpec) -> go.BaseTraceType:
    ...
    return go.Mesh3d(
        ...
        flatshading=False,
        lighting={"ambient": 0.35, "diffuse": 0.78, "specular": 0.12, "roughness": 0.48},
    ...
    *,
    ...
    for fig_spec in specs:
        ...
        for _, group_surfaces in groups.items():
            ...
            base = make_material_surface(fig_spec.hamiltonian, first, chunk_size=chunk_size)
            gap = base.band_gap
...
def build_plotly_gallery(
    ...
    rows = [spec for spec in specs if any(f"{spec.key}:{s.key}" in plot_meshes for s in spec.surfaces)]
    cols = max(len(spec.surfaces) for spec in rows)
    titles = []
    for spec in rows:
        for surface in spec.surfaces:
            if f"{spec.key}:{surface.key}" in plot_meshes:
                titles.append(f"{spec.title}<br>{surface.title}<br>cut={surface.gap_tol:g} eV")
            else:
                titles.append("")
    ...
    for row, fig_spec in enumerate(rows, start=1):
        for col, surface in enumerate(fig_spec.surfaces, start=1):
            ...
            scene_layouts[scene_name] = {
                "xaxis": {"title": "kx (1/A)", "range": list(bounds[0]), "showbackground": False, "gridcolor": "#d9d9d9", "zeroline": False},
                "yaxis": {"title": "ky (1/A)", "range": list(bounds[1]), "showbackground": False, "gridcolor": "#d9d9d9", "zeroline": False},
                "zaxis": {"title": "kz (1/A)", "range": list(bounds[2]), "showbackground": False, "gridcolor": "#d9d9d9", "zeroline": False},
                "aspectmode": "cube",
                "camera": camera,
            }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=THIS_REPO / "tmp" / "multiband_material_surface_check",
