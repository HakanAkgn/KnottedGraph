...
It keeps the source/output outside ``KnottedGraph_1earlier`` while importing
that checkout's production skeletonization, spatial-graph, and Yamada tools.
...
class TPMSFamily:
    ...
    domain: SolidDomain
    convention: str = (
        "Omega(lambda,c) = {r : F_lambda(r) <= c and Phi_domain(r) <= 0}"
    )
    rationale: str = ""
    ...
    *,
    ...
    if kind == "cylinder":
        ...
        return SolidDomain(
            ...
            formula=(
                f"max(sqrt(x^2+y^2)/{radius:.6g} - 1, "
                f"|z|/{half_height:.6g} - 1) <= 0"
            ),
...
def phase_grid(
    ...
    label_lookup = {
        idx + 1: next(
            r.phase_label for r in records if r.phase_signature == signature
        )
        for idx, signature in enumerate(signatures)
    }
    lookup = {
        (round(r.threshold_c, 12), round(r.lam, 12)): r
        for r in records
    }
...
def verify_endpoints(
    ...
    *,
    ...
    for family in families:
        ...
        domain_values = family.domain.sample(x, y, z)
        expected_samples = {
            key: field.sample(x, y, z) for key, field in fields.items()
        }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=THIS_REPO / "tmp" / "tpms_compact_scaffold_phase_maps",
    )
